...
const KreirajFiguricu = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await kreirajFiguricu(figuricaData, slika);
  ...
  return (
    <div className="kreiraj-figuricu-page">
      ...
      <form onSubmit={handleSubmit} className="kreiraj-figuricu-form">
        <input type="text" placeholder="Naslov figurice *" value={naslov} onChange={(e) => setNaslov(e.target.value)} required />
        <textarea placeholder="Opis figurice" value={opis} onChange={(e) => setOpis(e.target.value)}></textarea>
        <input type="number" placeholder="Cena figurice (RSD) *" value={cena} onChange={(e) => setCena(e.target.value)} required />
        
        <select value={kategorija} onChange={(e) => setKategorija(e.target.value)} required>
        ...
        <select value={stanje} onChange={(e) => setStanje(e.target.value)} required>
        ...
        <button type="submit" disabled={loading}>{loading ? 'Kreiranje...' : 'Kreiraj Figuricu'}</button>
      </form>
